...
std::cout << "============================================================================\n";
  int verbose = 10;
  std::string in_devname = "/dev/video0";

//  DeviceInterface_Custom *DC;
//  DC = DeviceInterface_Custom::Create(buf,bufSize,640,480,in_devname.c_str(),V4L2_PIX_FMT_YUYV,30,verbose);
//
//  ByteStreamMemoryBufferSource* Source = ByteStreamMemoryBufferSource::createNew(*env, buf, *bufSize);
//  if (Source == NULL) {
//    *env << "Unable to open file as a byte-stream file source\n";
//    exit(1);
//  }

  DeviceParameters_Custom param(in_devname.c_str(),0,0,0,0,verbose);
  DeviceSource_Custom* Source = DeviceSource_Custom::createNew(*env,param);
  if (Source == NULL) {
	  *env << "Unable to open as a byte-stream source\n";
	  exit(1);
  }
